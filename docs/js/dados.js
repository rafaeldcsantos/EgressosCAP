...
const totais = Object.fromEntries(["nome", "nivel", "ano", "foto", "lattes", "email", "email-alternativo", "instagram", "linkedin"].map((campo) => [campo, document.querySelector(`#total-${campo}`)]));
let egressos = [];
let campoOrdem = "nome";
...
function criarLinha(egresso) {
  ...
  const [rotuloFoto, classeFoto] = FOTOS[egresso.foto_tipo] || FOTOS.pendente;
...
function comparar(a, b) {
  const primeiro = campoOrdem === "ano_conclusao" ? Number(a[campoOrdem]) || 0 : normalizar(String(a[campoOrdem] || ""));
  const segundo = campoOrdem === "ano_conclusao" ? Number(b[campoOrdem]) || 0 : normalizar(String(b[campoOrdem] || ""));
...
function atualizarTotais(registros) {
  ...
  resumoContagem(totais.nivel, "Nível", { M: registros.filter((item) => item.nivel === "Mestrado").length, D: registros.filter((item) => item.nivel === "Doutorado").length });
  const anos = [...new Set(registros.map((item) => item.ano_conclusao))].sort((a, b) => a - b);
  preencherResumo(totais.ano, "Conclusão", anos.length ? [`${anos.length} anos`, `${anos[0]}–${anos.at(-1)}`] : ["—"]);
  ...
  [["lattes", "lattes_id", "ID Lattes"], ["email", "email", "E-mail"], ["email-alternativo", "email_alternativo", "E-mail alt."], ["instagram", "instagram", "Instagram"], ["linkedin", "linkedin", "LinkedIn"]].forEach(([target, campo, titulo]) => {
...
function atualizar() {
  ...
  const visiveis = egressos.filter((egresso) => (!termo || normalizar(egresso.nome).includes(termo)) && (!filtroAno.value || String(egresso.ano_conclusao) === filtroAno.value) && (!apenasPendentes.checked || pendencias(egresso).length)).sort(comparar);
...
async function iniciar() {
  ...
  [...new Set(egressos.map((egresso) => egresso.ano_conclusao))].sort((a, b) => b - a).forEach((ano) => filtroAno.add(new Option(ano, ano)));
  atualizar();
}
busca.addEventListener("input", atualizar);
filtroAno.addEventListener("change", atualizar);
